"""
名片辨識系統 - Streamlit 網頁版 v16 (已加入密碼保護)
支援手機和電腦使用 + 圖片旋轉功能
...
def main():
    ...
    # 【修改 1/3: 密碼設定與 Session State】
    SECRET_PASSWORD = "YZsz45;#"  # <<< 請將此處替換為您的密碼！
    ...
    with st.sidebar:
        ...
        if st.button("登入管理"):
            if password_input == SECRET_PASSWORD:
                ...
                # 登入成功後不需要 rerun，避免重設密碼欄位，讓使用者體驗更好
            else:
        ...
        if st.session_state.authenticated:
            ...
            if st.button("登出管理"):
                ...
                st.rerun() # 登出需要清除 show_all 狀態並重新整理
    ...
    if 'reader' not in st.session_state:
        with st.spinner('正在載入 OCR 模型(首次啟動需要下載,約1-2分鐘)...'):
            st.session_state.reader = load_ocr_reader()
            st.session_state.scanner = BusinessCardScanner(st.session_state.reader)
    ...
    if uploaded_file is not None:
        ...
        if 'card_info' in st.session_state:
            st.markdown("---")
            st.subheader("✏️ 編輯資料")
            ...
            with col1:
                ...
                country = st.selectbox("國家", 
                    options=['', 'Singapore', 'Malaysia', 'Thailand', 'Indonesia', 'Vietnam', 'Philippines', 'Brunei', 'Myanmar', 'Cambodia', 'Laos', 'Taiwan', 'Hong Kong', 'Japan', 'South Korea', 'China', 'India', 'Australia', 'New Zealand', 'United States', 'United Kingdom', 'Other'],
                    index=0 if not st.session_state.card_info.get('country', '') else 
                          ['', 'Singapore', 'Malaysia', 'Thailand', 'Indonesia', 'Vietnam', 'Philippines', 'Brunei', 'Myanmar', 'Cambodia', 'Laos', 'Taiwan', 'Hong Kong', 'Japan', 'South Korea', 'China', 'India', 'Australia', 'New Zealand', 'United States', 'United Kingdom', 'Other'].index(st.session_state.card_info.get('country', '')) if st.session_state.card_info.get('country', '') in ['', 'Singapore', 'Malaysia', 'Thailand', 'Indonesia', 'Vietnam', 'Philippines', 'Brunei', 'Myanmar', 'Cambodia', 'Laos', 'Taiwan', 'Hong Kong', 'Japan', 'South Korea', 'China', 'India', 'Australia', 'New Zealand', 'United States', 'United Kingdom', 'Other'] else 0
                )
            ...
            if st.button("💾 存入 Excel", type="primary"):
                ...
                try:
                    ...
                    del st.session_state.card_info
    ...
    # 【修改 3/3: 保護資料顯示區塊】
    # 只有成功登入 (authenticated=True) 且點擊 '查看所有名片' (show_all=True) 才顯示
            ...
            if st.session_state.get('edit_mode', False):
                ...
                if selected:
                    ...
                    with col1:
                        ...
                        edit_country = st.selectbox("國家", 
                            options=['', 'Singapore', 'Malaysia', 'Thailand', 'Indonesia', 'Vietnam', 'Philippines', 'Brunei', 'Myanmar', 'Cambodia', 'Laos', 'Taiwan', 'Hong Kong', 'Japan', 'South Korea', 'China', 'India', 'Australia', 'New Zealand', 'United States', 'United Kingdom', 'Other'],
                            index=['', 'Singapore', 'Malaysia', 'Thailand', 'Indonesia', 'Vietnam', 'Philippines', 'Brunei', 'Myanmar', 'Cambodia', 'Laos', 'Taiwan', 'Hong Kong', 'Japan', 'South Korea', 'China', 'India', 'Australia', 'New Zealand', 'United States', 'United Kingdom', 'Other'].index(row['country']) if row['country'] in ['', 'Singapore', 'Malaysia', 'Thailand', 'Indonesia', 'Vietnam', 'Philippines', 'Brunei', 'Myanmar', 'Cambodia', 'Laos', 'Taiwan', 'Hong Kong', 'Japan', 'South Korea', 'China', 'India', 'Australia', 'New Zealand', 'United States', 'United Kingdom', 'Other'] else 0,
                            key="edit_country"
                        )
